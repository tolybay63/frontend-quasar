<template>
  <q-dialog
    ref="dialog"
    @hide="onDialogHide"
    persistent
    autofocus
    transition-show="slide-up"
    transition-hide="slide-down"
  >
    <q-card class="q-dialog-plugin" style="width: 800px">
      <q-bar class="text-white bg-primary">
        <div>{{ $t("update") }}</div>
      </q-bar>

      <q-card-section>

        <div >
          {{ $t("group") }}:
          <span style="color: black; margin-left: 10px">
          <strong> {{ parentName }} </strong>
        </span>
        </div>

        <q-table
          color="primary"
          card-class="bg-amber-1"
          row-key="id"
          :columns="cols"
          :rows="rows"
          :wrap-cells="true"
          table-header-class="text-bold text-white bg-blue-grey-13"
          separator="cell" dense
          :rows-per-page-options="[0]"
        >
          <template v-slot:body="props">
            <q-tr :props="props">
              <td style="width: 5px">
                <q-btn
                  dense flat
                  color="blue"
                  :icon="
                    props.row.checked
                      ? 'check_box'
                      : 'check_box_outline_blank'
                  "
                  @click="selectedRow(props.row)"
                >
                </q-btn>
              </td>

              <q-td key="UserSecondName" :props="props">
                {{ props.row.UserSecondName }}
              </q-td>
              <q-td key="UserFirstName" :props="props">
                {{ props.row.UserFirstName }}
              </q-td>
              <q-td key="UserMiddleName" :props="props">
                {{ props.row.UserMiddleName }}
              </q-td>
            </q-tr>
          </template>

          <template v-slot:top>
            <q-btn
              :disable="loading"
              color="secondary" size="sm"
              dense icon="done_all"
              @click="fnCheckAll(true)"
              class="q-mr-lg"
            >
              <q-tooltip transition-hide="rotate" transition-show="rotate">
                {{ $t("checkAll") }}
              </q-tooltip>
            </q-btn>

            <q-btn
              :disable="loading"
              color="secondary" size="sm"
              dense icon="remove_done"
              @click="fnCheckAll(false)"
            >
              <q-tooltip transition-hide="rotate" transition-show="rotate">
                {{ $t("unCheckAll") }}
              </q-tooltip>
            </q-btn>

          </template>
        </q-table>
      </q-card-section>

      <q-card-actions align="right">
        <q-btn
          :loading="loading" dense
          color="primary" icon="save"
          :label="$t('save')"
          @click="onOKClick"
          :disable="validSave()"
        >
          <template #loading>
            <q-spinner-hourglass color="white"/>
          </template>
        </q-btn>

        <q-btn
          dense color="primary"
          icon="cancel"
          :label="$t('cancel')"
          @click="onCancelClick"
        />
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>

<script>
import {api} from "boot/axios";
import {ref} from "vue";
import {notifyError} from "src/utils/jsutils";

export default {
  props: ["obj", "cls", "parentName"],

  data() {
    return {
      cols: [],
      rows: [],
      loading: ref(false),
    };
  },

  emits: [
    // REQUIRED
    "ok",
    "hide",
  ],

  methods: {
    selectedRow(row) {
      row.checked = !row.checked;
    },

    fnCheckAll(ch) {
      this.rows.map((row) => {
        row.checked = ch;
      })
    },

    validSave() {
      return false;
    },

    getColumns() {
      return [
        {
          name: "checked",
          field: "checked",
          style: "width: 5%",
        },
        {
          name: "UserSecondName",
          label: this.$t("secondName"),
          field: "UserSecondName",
          align: "left",
          classes: "bg-blue-grey-1",
          headerStyle: "font-size: 1.2em",
          style: "width: 40%",
        },
        {
          name: "UserFirstName",
          label: this.$t("firstName"),
          field: "UserFirstName",
          align: "left",
          classes: "bg-blue-grey-1",
          headerStyle: "font-size: 1.2em",
          style: "width: 30%",
        },
        {
          name: "UserMiddleName",
          label: this.$t("middleName"),
          field: "UserMiddleName",
          align: "left",
          classes: "bg-blue-grey-1",
          headerStyle: "font-size: 1.2em",
          style: "width: 30%",
        },
      ];
    },

    // following method is REQUIRED
    // (don't change its name --> "show")
    show() {
      this.$refs.dialog.show();
    },

    onOKClick() {
      this.loading = ref(true);
      let dta = [];

      this.rows.forEach((r) => {
        if (r.checked) {
          dta.push({userobj: r.id, cls: r.cls});
        }
      });

      let err = false
      api
        .post('', {
          method: "entity/saveGroupUsers",
          params: [this.obj, /*this.cls, */dta],
        })
        .then(
          (response) => {
            this.$emit("ok", {res: true});
          },
          (error) => {
            err = true
            let msg = error.message;
            if (error.response.data.error.message)
              msg = error.response.data.error.message;
            notifyError(msg);
          }
        )
        .finally(() => {
          this.loading = ref(false);
          if (!err) this.hide();
        });
    },

    // following method is REQUIRED
    // (don't change its name --> "hide")
    hide() {
      this.$refs.dialog.hide();
    },

    onDialogHide() {
      // required to be emitted
      // when QDialog emits "hide" event
      this.$emit("hide");
    },

    onCancelClick() {
      // we just need to hide the dialog
      this.hide();
    },
  },
  created() {

    this.cols = this.getColumns();

    api
      .post('', {
        method: "data/loadUsersGroupAll",
        params: [this.obj],
      })
      .then((response) => {
        this.rows = response.data.result.records;
        //console.info("users", this.rows)
      })
  },
};
</script>

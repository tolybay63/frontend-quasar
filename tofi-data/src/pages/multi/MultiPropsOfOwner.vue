<template>
  ...
  <q-banner dense inline-actions class="bg-orange-1">
      <strong> {{ $t("itemsPropMatrixCharGr") }} </strong>
...
-->
  ...
  <div style="height: calc(100vh - 350px); width: 100%;" class="no-scroll">
            ...
            <td :data-th="cols[0].name" @click="toggle(item, index)">
              ...
              >
                ...
                <q-btn
                  dense
                  flat
                  :color="item.isItem === 0 ? 'gray' : 'blue'"
                  :icon="
                      selected.length > 0 && item.id === selected[0].id
                      ? 'check_box'
                      : 'check_box_outline_blank'
                  "
                  :disable="item.isItem===0"
...
export default {
  ...
  methods: {
    selectedCheck(item) {
      if (item.id.includes("ch_") || item.id.includes("mp_")) return;
      let th = this;
    ...
    loadDataProps(tr, isObj) {
      console.log("Multi: loadDataProps", tr, isObj)
      this.loading = ref(true);
      //let act = "typ/loadTypCharGrPropMatrix";

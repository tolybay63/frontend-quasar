  ...
  methods: {
    ...
    fnPropVal(row) {
      ...
      if (this.propType===allConsts.FD_PropType.factor)
        ...
        val = row.measure
      else if (this.propType===allConsts.FD_PropType.typ)
        val = row.obj
      else if (this.propType===allConsts.FD_PropType.reltyp)
        val = row.relobj
      //console.info("val********", val)
      if (val === 0) return null;
      if (!this.PropVal.get(val)) return null;

      if (allConsts.FD_VisualFormat.cod === this.requestParams.visualFormat)
        return this.PropVal.get(val).cod;
      else if (
        allConsts.FD_VisualFormat.short === this.requestParams.visualFormat
      )
        return this.PropVal.get(val).name;
      else if (
        allConsts.FD_VisualFormat.full === this.requestParams.visualFormat
      )
        return this.PropVal.get(val).fullName;
      else if (
        allConsts.FD_VisualFormat.cod_short === this.requestParams.visualFormat
      )
        return this.PropVal.get(val).cod + " - " + this.PropVal.get(val).name;
      else if (
        allConsts.FD_VisualFormat.cod_full === this.requestParams.visualFormat
      )
        return (
          this.PropVal.get(val).cod + " - " + this.PropVal.get(val).fullName
        );
      return this.PropVal.get(val).name;
    },

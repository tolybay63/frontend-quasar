  ...
  >
    <q-card class="q-dialog-plugin" style="width: 600px">
      ...
      <q-card-section>
        ...
        <!-- objDefectsComponent -->
        <q-select
          v-model="form['objClient']"
          :model-value="form['objClient']"
          :label="fmReqLabel('client')"
          :options="optClient"
          dense class="q-ma-md"
          map-options
          option-label="name"
          option-value="id"
          use-input
          @update:model-value="fnSelectClient"
          @filter="filterClient"
        />
...
export default {
  ...
  methods: {
    ...
    fnSelectClient(v) {
      this.form.objClient = v.id
      this.form.pvClient = v["pv"]
    },

    filterClient(val, update) {
      if (val === null || val === '') {
        update(() => {
          this.optClient = this.optClientOrg
        })
        return
      }
      update(() => {
        if (this.optClientOrg.length < 2) return
        const needle = val.toLowerCase()
        let name = 'name'
        this.optClient = this.optClientOrg.filter((v) => {
  ...
  created() {
    ...
    api
      .post('', {
        ...
        params: ['Cls_Client', "Prop_Client", 'clientdata'],
      })
      .then(
        (response) => {
          this.optClient = response.data.result["records"]
          this.optClientOrg = response.data.result["records"]
        })
      .finally(()=> {
        this.loading = false
      })

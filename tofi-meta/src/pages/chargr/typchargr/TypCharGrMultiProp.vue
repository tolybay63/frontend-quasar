<template>
  ...
  >
    ...
    <template v-slot:action>
      ...
      <q-btn
          v-if="hasTarget('mdl:mn_ds:typ:sel:char:matrix:edit')"
      ...
      <q-btn
          v-if="hasTarget('mdl:mn_ds:typ:sel:char:matrix:upd')"
          :dense="dense"
          icon="edit"
          color="secondary"
          class="q-ml-sm"
...
export default {
  name: "TypCharGrMultiProp",

  props: ["typCharGr", "dense"],
  ...
  methods: {
    ...
    disableEdit() {
      return !!(this.loading || this.selected.length === 0);
    },

    editRow(rec) {
      this.$q
          .dialog({
            component: UpdateTypCharGrMultiProp,
            componentProps: {
              data: rec,
              lg: this.lang,
              dense: true,
    ...
    editData() {
      this.$q
          .dialog({
            component: UpdateTypCharGrMultiPropUpd,
            componentProps: {
